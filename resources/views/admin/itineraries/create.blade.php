    ...
    <form method="POST" action="{{ route('itineraries.store') }}" enctype="multipart/form-data">
        ...
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="day" id="day" placeholder="Ngày trong lịch trình" value="{{ old('day') }}">
                @error('day')
                <div class="alert alert-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" name="activity_description" id="activity_description" rows="3" placeholder="Mô tả hoạt động">{{ old('activity_description') }}</textarea>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="location" id="location" placeholder="Địa điểm" value="{{ old('location') }}">
                @error('location')
                <div class="alert alert-danger">{{ $message }}</div>
                @enderror
            </div>
