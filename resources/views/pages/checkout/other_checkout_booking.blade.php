                    ...
                    <div id="mda-tour">
                        <div class="mda-info">
                            <div class="mda-info-img">
                                <img src="{{ asset('public/uploads/tours/' . $tour->image) }}" alt="{{ $tour->title_tours }}" style="width:650px; height:350px">
                            </div>
                            ...
                            <div class="mda-info-list">
                                <div class="mda-row">
                                    ...
                                    <span style="font-weight:bold">{{ $tour->tour_code }}</span>
                                </div>
                                <div class="mda-row">
                                    <span class="be">Thời gian:</span> 
                                    <span style="font-weight:bold">{{ $tour->tour_time }}</span>
                                </div>
                                <input type="hidden" id="total-all-hidden" value="{{ $departureData->price ?? 0 }}">
                                <div class="mda-row">
    <span class="be">Giá:</span>
    <span style="font-weight:bold" id="total-all">
        {{ number_format($tourPriceDetail->adult_price ?? 0, 0, ',', '.') }} đ
    </span>
</div>
                          <div class="mda-row">
                          <div class="mda-row">
                                        <span class="be">Ngày khởi hành:</span>
                                        <span class="font-weight-bold">{{ session('departure_date') ? \Carbon\Carbon::parse(session('departure_date'))->format('d/m/Y') : 'N/A' }}</span>
                                    </div>
</div>
                                <div class="mda-row">
                                    <span class="be">Nơi khởi hành:</span>
                                    <span style="font-weight:bold">{{ $tour->tour_from }}</span>
                                </div>
                                <div class="mda-row">
                                    <span class="be">Số chổ còn nhận:</span>
                                     @if($departureDate)
                                     <span id="seatsLeftDisplay" style="font-weight: bold;">Còn {{ $departureDate->available_seats ?? '0' }} chỗ</span>
                                  @else
                        ...
                        <ul class="mda-list-design">
                            <li>
                                <span style="color:#FF0000">Các khoản phí phát sinh (nếu có) như: phụ thu dành cho khách nước ngoài, việt kiều; phụ thu phòng đơn; phụ thu chênh lệch giá tour…</span>
                            </li>
        ...
        <div class="mda-table-container">
            <table class="mda-table chitiet_gia">
                <tbody>
                    ...
                    <tr>
                        <td><span><b>Giá cho ngày {{ \Carbon\Carbon::parse($departureDate->departure_date)->format('d/m/Y') }}</b></span></td>
                        <td>
                            <span class="mda-money">
                                {{ $tourPriceDetail ? number_format($tourPriceDetail->adult_price, 0, ',', '.') : 'Chưa có giá' }}
                            </span> đ
                        </td>
                        <td>
                            <span class="mda-money">
                                {{ $tourPriceDetail ? number_format($tourPriceDetail->child_price, 0, ',', '.') : 'Chưa có giá' }}
                            </span> đ
                        </td>
                        <td>
                            <span class="mda-money">
                                {{ $tourPriceDetail ? number_format($tourPriceDetail->infant_price, 0, ',', '.') : 'Chưa có giá' }}
                            </span> đ
                        </td>
                        <td>
                            <span class="mda-money">
                                {{ $tourPriceDetail ? number_format($tourPriceDetail->baby_price, 0, ',', '.') : 'Chưa có giá' }}
                            </span> đ
                        </td>
                    </tr>
                    ...
                    @if($tourPriceDetail)
                    <tr>
                        <td><span><b>Phụ thu Nước Ngoài</b></span></td>
                        <td>
                            <span class="mda-money">
                                {{ number_format($tourPriceDetail->foreign_surcharge, 0, ',', '.') }} đ
                            </span>
                        </td>
                    ...
                    <tr>
                        ...
                        <td colspan="4" style="text-align: center;">
                            <span class="mda-money">
                                {{ number_format($tourPriceDetail->single_room_surcharge, 0, ',', '.') }} đ
                            </span>
                        </td>
                    </tr>
                    @endif
...
<div class="row">
    ...
    <input type="hidden" name="coupon_code" id="coupon_code" value="">
            ...
            <div class=" form-group col-md-3" style="margin-top: 30px;">
               ...
               <span class="mda-price-summary" style="font-size: 20px;line-height: 28px;color: #ed0080;font-weight: 600;" id="final-price">0 đ</span>
            </div>
            <div class="form-group col-md-3"style="margin-top: 25px;" >
    ...
    <div id="seat-warning" style="color: red; font-weight: bold;"></div>
